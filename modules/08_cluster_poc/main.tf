...
locals {
  ...
  validation_errors = compact(concat(
    ...
    // Requires
    var.instance_size != null && var.regions == null ? ["Cannot use var.instance_size without var.regions"] : [],
    var.auto_scaling != null && var.regions == null ? ["Cannot use var.auto_scaling without var.regions"] : [],
    var.auto_scaling_analytics != null && var.regions == null ? ["Cannot use var.auto_scaling_analytics without var.regions"] : [],

    // Per-region invalid instance_size when autoscaling is used
    var.auto_scaling != null ? [for idx, r in local.regions : r.instance_size != null ? "Cannot use regions[*].instance_size when auto_scaling is used: index ${idx} instance_size=${r.instance_size}" : ""] : [],
