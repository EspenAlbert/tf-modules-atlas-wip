...
resource "mongodbatlas_encryption_at_rest" "this" {
  dynamic "aws_kms_config" {
    for_each = var.aws_kms_config == null ? [] : var.aws_kms_config
    content {
  ...
  dynamic "azure_key_vault_config" {
    for_each = var.azure_key_vault_config == null ? [] : var.azure_key_vault_config
    content {
  ...
  dynamic "google_cloud_kms_config" {
    for_each = var.google_cloud_kms_config == null ? [] : var.google_cloud_kms_config
    content {
