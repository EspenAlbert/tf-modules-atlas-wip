...
class Role:
    ...
    REQUIRED_ATTRIBUTES: ClassVar[Set[str]] = set()
    COMPUTED_ONLY_ATTRIBUTES: ClassVar[Set[str]] = {"project_role_assignments"}
    DEFAULTS_HCL_STRINGS: ClassVar[dict[str, str]] = {}
    org_roles: Optional[List[str]] = None
    project_role_assignments: Optional[List[ProjectRoleAssignment]] = None

    def __post_init__(self):
        if self.project_role_assignments is not None:
            self.project_role_assignments = [
                x if isinstance(x, ProjectRoleAssignment) else ProjectRoleAssignment(**x)
                for x in self.project_role_assignments
            ]
...
class Resource:
    ...
    REQUIRED_ATTRIBUTES: ClassVar[Set[str]] = {"org_id", "roles", "username"}
